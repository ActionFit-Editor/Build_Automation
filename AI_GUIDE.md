...
- Current package version at generation time: `1.0.13`
...
- Workflow secret validation calls the project-root `.github/scripts/validate-local-runner-secrets.sh`. The package keeps the source copy at `.github/scripts/validate-local-runner-secrets.sh`, and the AutoBuild workflow sync button copies it into the consuming project together with the workflow yml. Do not make pre-Unity shell steps depend on `Packages/` or `Library/PackageCache`.
...
- AutoBuild window workflow sync button copies `WorkflowTemplates/buildcommit-auto-build.yml` to `.github/workflows/buildcommit-auto-build.yml` and package `.github/scripts/validate-local-runner-secrets.sh` to project `.github/scripts/validate-local-runner-secrets.sh` after confirmation when either target differs.
