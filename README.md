...
이 패키지는 `com.actionfit.buildsetting`의 빌드 설정과 `com.actionfit.customsymbols`의 build 체크 심볼을 사용합니다. Build Automation은 버전, 번들 번호, 플랫폼, 앱 식별자 같은 비밀이 아닌 빌드 메타데이터만 원격 CI 요청에 저장하고 signing/upload credential은 self-hosted runner에서만 읽습니다.
...
```json
{
  "dependencies": {
    ...
    "com.actionfit.buildautomation": "https://github.com/ActionFit-Editor/Build_Automation.git#1.0.34"
  }
}
```
...
Android 요청에는 비밀이 아닌 `androidKeyaliasName`만 빌드 메타데이터로 저장할 수 있습니다. Keystore 파일/base64, keystore 및 alias 비밀번호, Google Play JSON, iOS team credential, App Store Connect API key, certificate와 keychain 비밀번호는 BuildRequest에 저장하지 않습니다. Android 빌드는 runner의 `ANDROID_KEYSTORE_PATH`, `ANDROID_KEYSTORE_PASS`, `ANDROID_KEYALIAS_PASS`를 필수로 사용하고, `ANDROID_KEYALIAS_NAME`이 있으면 request alias보다 우선합니다.
...
`자동 빌드 심볼 세팅`은 `BuildAutomationSettingsSO.autoConfigureBuildSymbols`에 저장되고 schema 11의 `autoConfigureBuildSymbols`로 전달됩니다. 활성화하면 `SwitchToRequestBuildTarget` 프로세스가 `CustomSymbolsSO.GetBuildSymbols(BuildTarget)` 결과를 대상 플랫폼의 scripting define symbols에 먼저 저장하고 종료합니다. 다음 `BuildFromRequest` Unity 프로세스는 새 심볼로 재컴파일된 상태에서 시작하며, 실제 빌드 전에 현재 심볼이 예상 목록과 정확히 일치하는지 검증합니다. 설정 에셋이 없거나 심볼이 불일치하면 빌드를 실패시킵니다.
...
Android keystore와 signing 비밀번호, Google Play service account, iOS team id, App Store Connect API key, certificate, keychain password, Slack webhook URL은 모두 Mac runner의 로컬 시크릿 번들에서 읽습니다. 요청 JSON 값으로 fallback하지 않습니다. `CI_SECRET_ROOT` 환경변수를 지정하지 않으면 `$HOME/ci-secrets/build-automation`을 사용합니다.
...
- `com.actionfit.customsymbols@1.0.5` 이상
- Unity `6000.2`
