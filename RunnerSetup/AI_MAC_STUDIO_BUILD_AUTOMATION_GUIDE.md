...
BuildRequest carries the project-specific Android keystore Base64 and signing passwords. The shared bundle selected by `CI_SECRET_ROOT` provides Android fallbacks plus upload credentials, team ids, Apple Distribution `.p12` files, App Store provisioning profiles, optional keychain settings, and Slack credentials. BuildRequest never carries runner-local paths or Slack credentials. Trusted Unity runners post notifications and Development APKs directly.
...
- `.github/scripts/resolve-local-secret-root.sh`: resolves the Mac Studio local bundle or MacBook SMB mount and exports `CI_SECRET_ROOT`.
- `.github/scripts/notify-slack-build-result.sh` and `.github/scripts/upload-slack-file.sh`: send direct notifications and Development APK files.
...
Mac Studio runners normally use this location. MacBook runners must mount the Mac Studio `ActionFitBuildAutomation` SMB share at a stable path and set that path as `CI_SECRET_ROOT` in each runner service environment. The resolver has `/Volumes/ActionFitBuildAutomation` only as a fallback; do not add any local path to BuildCommit request JSON.
...
4. Run `gh auth setup-git --hostname github.com` as the runner user, or put a read-only package token at `$CI_SECRET_ROOT/shared/github-package-read-token`.
5. Put `slack-bot-token` and `slack-channel-id` under the shared bundle's `shared/` directory. The Bot needs `chat:write`, `files:write`, and channel membership. `slack-webhook-url` may remain for older package versions but is not used by the current helpers. Slack member mentions remain request metadata: AutoBuild reads shared `BuildAutomationSettingsSO` mention rows and serializes only `Mention`-enabled Member ID values into `.build/build_request.json` as the `slackMentions` JSON array. AutoBuild memo values stay in the shared SO and are not committed into the request JSON.
...
Slack delivery runs in the affinity `mobile-build` job. The job posts advisory start and result messages through the same Bot and channel used for APK delivery, builds every requested platform, completes final cleanup and result aggregation, and then uploads the fresh Development APK directly from its local build path. The APK post carries the success message, so no duplicate success message is sent. Slack credential, network, or API failure stays advisory and attempts a `BUILD SUCCESS / APK DELIVERY FAILED` message without changing a successful build conclusion. A completion timeout is treated as indeterminate: shared durable state blocks duplicate uploads and the workflow avoids a contradictory failure post until an operator reconciles the logged receipt. Development APKs never enter GitHub Artifact storage.
...
- If automatic build symbols fail, verify `autoConfigureBuildSymbols`, `com.actionfit.customsymbols@1.0.7`, and `CustomSymbolsSO` platform/Build checks. Missing settings are created from the current project defines before the target-switch process prepares symbols; the separate build process then verifies them.
...
- If Slack notifications do not arrive, inspect `Resolve shared runner secret root`, `Notify Slack build start`, `Attach Development APK directly to Slack`, and `Notify Slack final result` in the source build. A `chat.postMessage` rejection reports a sanitized `error=<code>` value; use `not_in_channel`, `missing_scope`, `invalid_auth`, or `channel_not_found` to distinguish membership, scope, token, and channel configuration failures. Verify the shared root is mounted, `slack-bot-token` and `slack-channel-id` are readable, the Bot has `chat:write`, `files:write`, and channel membership, and the APK path came from the current Android action output. If notifications arrive without mentions, check `.build/build_request.json` `slackMentions` and use Slack member IDs such as `U12345678`, not display names.
- If direct delivery reports an unresolved completion attempt, inspect the channel for the logged Slack file ID before changing anything. Mark an existing post delivered with the receipt manager; only when the post is definitely absent may an operator remove the exact logged receipt JSON and rerun. Never age-delete pending receipts.
...
- Do not expose the shared bundle to `ci`, validation, allocator, or untrusted pull request workflows. Direct Slack delivery intentionally makes its credentials readable to trusted `unity-mobile` runner processes.
...
- Do not describe file mode `600` as isolation between runners that use the same macOS account or shared SMB identity.
