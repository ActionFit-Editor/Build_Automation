...
5. Put `slack-webhook-url`, `slack-bot-token`, and `slack-channel-id` under the shared bundle's `shared/` directory. The Bot needs `files:write` and channel membership. Slack member mentions remain request metadata: AutoBuild reads shared `BuildAutomationSettingsSO` mention rows and serializes only `Mention`-enabled Member ID values into `.build/build_request.json` as the `slackMentions` JSON array. AutoBuild memo values stay in the shared SO and are not committed into the request JSON.
...
Slack delivery runs in the affinity `mobile-build` job. The job sends an advisory start webhook, builds every requested platform, completes final cleanup and result aggregation, and then uploads the fresh Development APK directly from its local build path. The APK post carries the success message, so no duplicate success webhook is sent. Slack credential, network, or API failure stays advisory and attempts a `BUILD SUCCESS / APK DELIVERY FAILED` webhook without changing a successful build conclusion. A completion timeout is treated as indeterminate: shared durable state blocks duplicate uploads and the workflow avoids a contradictory failure post until an operator reconciles the logged receipt. Development APKs never enter GitHub Artifact storage.
