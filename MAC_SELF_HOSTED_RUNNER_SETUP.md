...
조직 runner 조회와 라벨 추가를 위해 Actions secret `UNITY_RUNNER_ALLOCATOR_TOKEN`을 준비합니다. GitHub App installation token 또는 fine-grained PAT에는 조직 `Self-hosted runners: write` 권한이 필요합니다. 가능하면 선택된 private repository에만 노출되는 조직 secret을 사용하고, 일반 `GITHUB_TOKEN`이나 runner 로컬 build credential을 재사용하지 않습니다. allocator는 token 값을 로그에 출력하지 않으며 BuildRequest를 체크아웃하지 않습니다.

allocator는 기존 프로젝트 라벨이 붙은 online `unity-mobile` runner를 재사용합니다. 매핑이 없을 때만 online·idle runner 중 `ci`, `unity-package-ci`가 없고 `project-*` 라벨 수가 가장 적은 Mac에 라벨을 추가합니다. workflow의 `mobile-build` job은 allocator output과 `self-hosted`, `macOS`, `unity-mobile` 조합으로 이 Mac을 선택합니다.
...
- `Allocate Project Affinity Runner` job에서 `UNITY_RUNNER_ALLOCATOR_TOKEN` 누락 또는 조직 runner API 403이 발생하지 않았는지 확인
- allocator token에 조직 `Self-hosted runners: write` 권한이 있는지 확인
