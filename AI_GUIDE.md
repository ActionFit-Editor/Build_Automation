...
- Current package version at generation time: `1.0.51`
...
- The Slack delivery host bundle lives under `/Users/lydia/workspace/slack-delivery`: tools in `bin`, credentials in `secrets/shared`. Only this runner reads `slack-webhook-url`, `slack-bot-token`, and `slack-channel-id`. Delivery warnings, missing credentials, artifact download failures, and Slack API failures are advisory and must never alter the source BuildCommit result. During rollout the runner group uses selected-repository access limited to trusted BuildCommit repositories. After every delivery workflow exists on its default branch, enable `restricted_to_workflows=true` and allow only exact `<org>/<repo>/.github/workflows/buildcommit-slack-delivery.yml@refs/heads/main` entries. GitHub rejects a selected-workflow entry before that workflow file exists, so do not reverse this rollout order.
