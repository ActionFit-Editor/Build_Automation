...
`self-hosted`, `macOS`, architecture 라벨은 GitHub runner가 자동으로 붙입니다. 중앙 allocator는 프로젝트별 `project-*` affinity 라벨을 추가합니다. 저장소 Actions variable `UNITY_RUNNER_AFFINITY_LABEL`에는 배정된 라벨(예: `project-cat-merge-cafe`)을 설정하며, workflow는 `self-hosted`, `macOS`, `unity-mobile`, 해당 affinity 라벨 조합으로 이 Mac을 선택합니다.
...
- 저장소 variable `UNITY_RUNNER_AFFINITY_LABEL` 값이 배정된 `project-*` 라벨과 일치하는지 확인
- workflow의 `self-hosted`, `macOS`, `unity-mobile`, affinity 라벨이 모두 runner에 있는지 확인
