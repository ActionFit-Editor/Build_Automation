...
```json
{
  "dependencies": {
    ...
    "com.actionfit.buildautomation": "https://github.com/ActionFit-Editor/Build_Automation.git#1.0.54"
  }
}
```
...
`Development Build`는 연결된 `BuildSettingsSO.developmentBuild`에 저장되는 기본 OFF 옵션입니다. Custom Symbols 설치 여부와 관계없이 AutoBuild 창에 표시되며 기존 `DEV` scripting define 설정과는 독립적입니다. 요청을 생성할 때 값을 고정해 두고, CI가 실제 Android/iOS 빌드 직전에 설정에 적용합니다. 활성화하면 Android working request는 APK와 Store 미업로드로 강제되고, 새로 생성된 APK를 고유한 GitHub Artifact로 보관합니다. 별도 Slack delivery workflow가 성공한 원본 실행에서 이 Artifact를 내려받아 성공 메시지와 APK를 하나의 Slack 파일 게시물로 전송합니다. iOS working request는 마케팅 버전을 유지하고 빌드 번호를 `1`부터 시작합니다. CI가 같은 마케팅 버전의 완료된 빌드와 진행 중 업로드를 조회해 이미 사용된 최대 번호보다 1 큰 값을 작업용 요청에 적용한 뒤 TestFlight에 올립니다. 원본 BuildRequest와 BuildSettingsSO의 번들 번호는 변경하지 않습니다.
...
8. `Development Build`를 확인합니다. 기본 OFF이며 켜면 CI Android/iOS 빌드 옵션에 `BuildOptions.Development`가 추가되고 Android APK Artifact/전용 Slack 전달, iOS TestFlight 빌드번호 1부터의 자동 증가 정책이 적용됩니다.
