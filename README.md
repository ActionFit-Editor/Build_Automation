...
```json
{
  "dependencies": {
    ...
    "com.actionfit.buildautomation": "https://github.com/ActionFit-Editor/Build_Automation.git#1.0.63"
  }
}
```
...
8. `Development Build`를 확인합니다. 기본 OFF이며 켜면 `Build Number` 입력이 비활성화됩니다. Android/Both BuildCommit은 현재 `bundleNo`를 1 증가시켜 BuildSettingsSO, BuildRequest, 커밋과 태그에 함께 저장하므로 앱 버전이 바뀌어도 Android 번호가 계속 증가합니다. iOS는 마케팅 버전마다 TestFlight 빌드번호를 1부터 자동 증가시킵니다. CI Android/iOS 빌드 옵션에는 `BuildOptions.Development`가 추가되고 Android APK는 Slack으로 직접 전달됩니다.
