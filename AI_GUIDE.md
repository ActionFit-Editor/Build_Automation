...
- Current package version at generation time: `1.0.34`
...
This package depends on `com.actionfit.buildsetting`, `com.actionfit.customsymbols`, and `com.actionfit.githubauth`. Keep build/player settings in Build Setting and build symbol selection in Custom Symbols. Keep CI request orchestration and remote build workflow behavior in Build Automation.
...
- `androidKeyaliasName` may remain in the request as non-secret build metadata. Android keystore bytes/path and signing passwords must never be serialized. Android CI requires runner-provided `ANDROID_KEYSTORE_PATH`, `ANDROID_KEYSTORE_PASS`, and `ANDROID_KEYALIAS_PASS`; optional runner `ANDROID_KEYALIAS_NAME` overrides the request alias.
...
- No signing/upload credential may be present in `.build/build_request.json`. This includes Android keystore bytes/passwords, Google Play JSON, iOS team credential, App Store Connect identifiers/P8, Apple Distribution `.p12`, provisioning profiles, certificate passwords, and keychain passwords. Legacy credential-shaped JSON keys must be ignored and removed from per-job request copies, never used as fallback.
- The authoritative credential source is the Mac runner local secret bundle selected by the `CI_SECRET_ROOT` environment variable; the scripts default to `$HOME/ci-secrets/build-automation`. The bundle provides required Android signing values plus `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON_PATH`, `IOS_DEVELOPMENT_TEAM_ID`, `APP_STORE_CONNECT_API_KEY_ID`, `APP_STORE_CONNECT_ISSUER_ID`, `APP_STORE_CONNECT_API_KEY_P8_PATH`, `IOS_DISTRIBUTION_CERTIFICATE_P12_PATH`, `IOS_DISTRIBUTION_CERTIFICATE_PASSWORD`, `IOS_APP_STORE_PROVISIONING_PROFILE_DIR`, optional `IOS_APP_STORE_PROVISIONING_PROFILE_PATH`, `IOS_PROVISIONING_PROFILE_AUTO_GENERATE`, optional `IOS_KEYCHAIN_PASSWORD`, and optional `IOS_KEYCHAIN_PATH`. BuildCommit requests must not carry runner-local paths.
