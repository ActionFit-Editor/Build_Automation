...
```json
{
  "dependencies": {
    ...
    "com.actionfit.buildautomation": "https://github.com/ActionFit-Editor/Build_Automation.git#1.0.53"
  }
}
```
...
- GitHub Actions templates: `WorkflowTemplates/buildcommit-auto-build.yml`, `WorkflowTemplates/buildcommit-slack-delivery.yml`
- Build and host-tool script sources: `.github/scripts/resolve-unity-project.sh`, `.github/scripts/resolve-unity-editor.sh`, `.github/scripts/validate-local-runner-secrets.sh`, `.github/scripts/prepare-actionfit-private-package-access.sh`, `.github/scripts/notify-slack-build-result.sh`, `.github/scripts/upload-slack-file.sh`, `.github/scripts/check-testflight-build-number.rb`, `.github/scripts/cleanup-old-build-artifacts.sh`
...
- Slack delivery runner guide: `RunnerSetup/SLACK_DELIVERY_RUNNER_SETUP.md`
...
`Development Build`는 연결된 `BuildSettingsSO.developmentBuild`에 저장되는 기본 OFF 옵션입니다. Custom Symbols 설치 여부와 관계없이 AutoBuild 창에 표시되며 기존 `DEV` scripting define 설정과는 독립적입니다. 요청을 생성할 때 값을 고정해 두고, CI가 실제 Android/iOS 빌드 직전에 설정에 적용합니다. 활성화하면 Android working request는 APK와 Store 미업로드로 강제되고, 새로 생성된 APK를 고유한 GitHub Artifact로 보관합니다. 별도 Slack delivery workflow가 성공한 원본 실행에서 이 Artifact를 내려받아 성공 메시지와 APK를 하나의 Slack 파일 게시물로 전송합니다. iOS working request는 마케팅 버전을 유지하고 빌드 번호만 `1`로 고정해 TestFlight에 올리며, 같은 마케팅 버전의 `1`이 이미 확인되면 자동 증가 없이 명확히 실패합니다. 원본 BuildRequest와 BuildSettingsSO의 번들 번호는 변경하지 않습니다.
...
8. `Development Build`를 확인합니다. 기본 OFF이며 켜면 CI Android/iOS 빌드 옵션에 `BuildOptions.Development`가 추가되고 Android APK Artifact/전용 Slack 전달, iOS TestFlight 빌드번호 1 정책이 적용됩니다.
...
CI는 request의 `developmentBuild` 값을 실제 빌드 직전에 Build Setting reflection bridge로 적용합니다. 필요한 `BuildSettingsSO.developmentBuild` 계약이 없으면 최소 Build Setting 버전 안내와 함께 즉시 실패하며, Android/iOS 빌드 프로세스는 기존 플래그를 유지한 채 `BuildOptions.Development`를 추가합니다. Development 정책은 플랫폼 working request에만 적용되어 원본 요청을 보존합니다. Android는 현재 phase marker 이후 생성된 APK만 Artifact 대상으로 인정하고 Google Play를 건너뜁니다. Slack delivery workflow나 Slack API가 실패해도 원본 Artifact와 성공한 Unity 빌드 결과는 유지됩니다. iOS는 App Store Connect의 공식 `apps`, `builds`, `buildUploads` API로 완료되었거나 처리 중인 동일 버전의 빌드번호 `1` 충돌을 먼저 검사한 뒤 archive/upload합니다. API 인증, 응답 형식, 또는 조회가 실패하면 중복 없음으로 간주하지 않고 archive 전에 중단합니다.
...
TestFlight 업로드는 매 시도마다 새 임시 세션을 사용하며 기본 15분 hard timeout과 최대 2회 전체 프로세스 재시도를 적용합니다. `pilot` 또는 하위 `altool`이 네트워크 단절 뒤 내부 재시도에 머물면 process group을 종료하고 새 세션으로 한 번만 다시 시도합니다. workflow의 deferred Store worker는 기본 60분 상한, 전체 mobile job은 180분 상한을 사용하므로 업로드 재시도가 무한히 job을 점유하지 않습니다.

모바일 빌드 workflow template은 `WorkflowTemplates/buildcommit-auto-build.yml`, 전용 Slack 전달 template은 `WorkflowTemplates/buildcommit-slack-delivery.yml`에 있습니다. 플랫폼 단계 원본은 `.github/actions/build-android/action.yml`, `.github/actions/build-ios/action.yml`, workflow가 호출하는 script 원본은 패키지의 `.github/scripts/`에 있습니다. `resolve-unity-project.sh`는 request의 `unityProjectPath`로 Unity 프로젝트 디렉터리와 모든 파생 경로를 결정하고, `resolve-unity-editor.sh`는 해당 프로젝트의 `ProjectSettings/ProjectVersion.txt`에서 Unity 버전을 읽습니다. `prepare-actionfit-private-package-access.sh`는 해당 프로젝트의 `Packages/manifest.json`에 필요한 private package 접근을 준비하고, `validate-local-runner-secrets.sh`는 Unity Mac runner secret bundle을 검증합니다.

`AutoBuild` 창의 `Update GitHub Workflow` 버튼은 두 package workflow template을 Git 저장소 루트의 `.github/workflows/`, 그리고 build composite actions와 scripts를 `.github/actions/`, `.github/scripts/`로 복사합니다. 기존 파일이 template과 다르면 확인창을 띄운 뒤 덮어씁니다. `workflow_run` 기반 Slack 전달을 활성화하려면 `buildcommit-slack-delivery.yml`이 저장소 default branch에 먼저 포함되어야 합니다. 배포 중에는 `slack-delivery` runner group을 selected repositories로 제한하고, 각 default branch에 파일이 들어간 뒤 `restricted_to_workflows=true`와 정확한 `<org>/<repo>/.github/workflows/buildcommit-slack-delivery.yml@refs/heads/<default-branch>` allowlist를 최종 적용합니다.

Development Android APK Artifact는 Slack 전달을 위해 1일만 보존합니다. Delivery runner는 업로드 전에 source repository/run/attempt별 mode `600` 영수증을 `pending`으로 원자 생성하고, Slack 완료 확인 후 `delivered`로 전환합니다. cleanup 실패 후 workflow 전체를 다시 실행해도 delivered 영수증이 있으면 APK 다운로드와 Slack 재업로드를 생략하며, Artifact가 이미 없으면 cleanup은 성공으로 끝납니다. Slack 완료 요청 결과가 불확실하거나 로컬 commit 전에 중단되면 pending을 보존해 자동 중복 게시를 차단합니다. APK 누락이나 Slack 업로드 실패는 Unity 원본 빌드 결과를 변경하지 않지만 `BuildCommit Slack Delivery` workflow를 실패로 표시하고, Slack에는 `BUILD SUCCESS / APK DELIVERY FAILED`를 보냅니다. Store 업로드에 성공한 AAB/IPA는 해당 upload step의 정확한 outcome으로 판정해 GitHub에 중복 보관하지 않습니다. 이번 phase marker 이후 staging된 Store 실패 복구 산출물은 3일, 실패 진단 로그는 7일만 보존합니다. 복구 바이너리 Artifact 업로드는 필수 단계이므로 quota나 전송 오류를 성공으로 숨기지 않습니다.
...
Android keystore와 signing 비밀번호는 BuildRequest 값을 우선 사용하고, 값이 없을 때 Unity Mac runner의 로컬 시크릿 번들로 fallback합니다. Google Play service account, iOS team id, App Store Connect API key, certificate와 keychain password도 이 번들에서 읽습니다. 기본 workflow template은 기존 ActionFit runner bundle을 계속 사용하도록 `CI_SECRET_ROOT=/Users/lydia/workspace/build-automation`을 명시하며, setup/validation script를 workflow 밖에서 직접 실행해 이 환경변수가 없으면 `$HOME/ci-secrets/build-automation`으로 fallback합니다. Unity 빌드 러너는 Slack credential을 읽지 않습니다. Slack delivery runner만 `/Users/lydia/workspace/slack-delivery/secrets/shared`를 사용합니다. 수동 validator 실행은 credential 값을 출력하지 않으며, GitHub `::add-mask::` 명령은 `GITHUB_ACTIONS=true`일 때만 출력합니다.
...
상세 Mac 서버 준비 절차는 [MAC_SELF_HOSTED_RUNNER_SETUP.md](MAC_SELF_HOSTED_RUNNER_SETUP.md), Slack 전달 러너 절차는 [RunnerSetup/SLACK_DELIVERY_RUNNER_SETUP.md](RunnerSetup/SLACK_DELIVERY_RUNNER_SETUP.md)를 참고합니다.
