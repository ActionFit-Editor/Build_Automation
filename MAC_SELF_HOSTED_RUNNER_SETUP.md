...
- runner를 실행하는 macOS 사용자 계정에서 프로젝트 `ProjectSettings/ProjectVersion.txt`의 `m_EditorVersion`에 해당하는 Unity 라이선스가 활성화되어 있음
...
Unity Hub에서 대상 프로젝트의 `ProjectSettings/ProjectVersion.txt`에 적힌 `m_EditorVersion` 버전을 설치합니다.
...
self-hosted runner는 회사 credential을 GitHub Secrets가 아니라 Mac 로컬 파일에서 읽습니다. 기준 경로는 `.github/workflows/buildcommit-auto-build.yml`의 `CI_SECRET_ROOT`이고, 현재 yml 값은 `/Users/lydia/workspace/build-automation`입니다. BuildCommit request에는 runner 로컬 경로를 넣지 않습니다.
...
```bash
chmod -R go-rwx "$HOME/workspace/build-automation"
...
프로젝트 `ProjectSettings/ProjectVersion.txt`의 `m_EditorVersion`과 일치하는 Unity가 아래 경로에 없으면 workflow가 바로 실패합니다.
