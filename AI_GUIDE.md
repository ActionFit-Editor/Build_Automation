...
- Current package version at generation time: `1.0.35`
...
- The Mac runner local secret bundle selected by `CI_SECRET_ROOT` remains authoritative for Google Play, iOS, App Store Connect, certificate, provisioning profile, keychain, and Slack credentials. The scripts default to `$HOME/ci-secrets/build-automation`. Android signing values in the bundle are optional fallbacks for requests that omit one or more Android signing fields. BuildCommit requests must not carry runner-local paths.
