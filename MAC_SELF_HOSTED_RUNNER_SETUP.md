...
self-hosted runner는 회사 credential을 GitHub Secrets나 BuildRequest가 아니라 Mac 로컬 파일에서만 읽습니다. `CI_SECRET_ROOT`를 지정하지 않으면 기본값은 `$HOME/ci-secrets/build-automation`입니다. BuildCommit request에는 runner 로컬 경로나 credential을 넣지 않습니다.
...
두 값은 모든 Android BuildCommit job에서 필수입니다. Schema 11 request는 signing 비밀번호를 저장하지 않으며 legacy request의 credential 필드도 fallback으로 사용하지 않습니다.
...
workflow는 `shared/android-signing.env`를 먼저 읽고 `profiles/<profile>/android-signing.env`를 나중에 읽습니다. 따라서 프로필별 파일에 값이 있으면 그 값이 우선입니다. Request JSON은 비밀번호 source가 아닙니다.
...
Android keystore 파일과 signing 비밀번호는 runner 로컬 `ANDROID_KEYSTORE_PATH`, `ANDROID_KEYSTORE_PASS`, `ANDROID_KEYALIAS_PASS`에서만 전달하며 모두 필수입니다. 비밀이 아닌 alias는 AutoBuild request의 `androidKeyaliasName`을 사용할 수 있고, profile의 `ANDROID_KEYALIAS_NAME`으로 덮어쓸 수 있습니다. Keystore Base64와 비밀번호는 request에 직렬화하지 않습니다.
