...
- Current package version at generation time: `1.0.44`
...
- AutoBuild window workflow sync copies `WorkflowTemplates/buildcommit-auto-build.yml`, `.github/actions/build-android/action.yml`, `.github/actions/build-ios/action.yml`, and package `.github/scripts/*.sh` into the Git repository root `.github/`. Manual sync asks for confirmation; BuildCommit auto sync runs without confirmation when `Auto Sync Build Files` is enabled.
...
- The workflow requires repository variable `UNITY_RUNNER_AFFINITY_LABEL` to contain a project label such as `project-cat-merge-cafe`. Its single `mobile-build` job requests `self-hosted`, `macOS`, `unity-mobile`, and that project label so both platforms use the same runner-local workspace.
