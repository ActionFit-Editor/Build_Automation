...
`BuildAutomationSettingsSO`는 Editor 전용 설정으로 등록되며 canonical 경로는 `Assets/_Data/_BuildAutomation/BuildAutomationSettingsSO.asset`입니다. EditorPrefs가 기억한 에셋, canonical 에셋, 프로젝트 안의 단일 기존 에셋을 우선 재사용하고 없을 때만 생성합니다. 기존 빌드 값은 덮어쓰지 않습니다.
...
- 패키지 root: `Tools > Package > Build Automation`
...
`Development Build`는 연결된 `BuildSettingsSO.developmentBuild`에 저장되는 기본 OFF 옵션입니다. Custom Symbols 설치 여부와 관계없이 AutoBuild 창에 표시되며 기존 `DEV` scripting define 설정과는 독립적입니다. 요청을 생성할 때 값을 고정해 두고, CI가 실제 Android/iOS 빌드 직전에 설정에 적용합니다. Development Android/Both BuildCommit은 현재 브랜치 값뿐 아니라 저장소 전체의 `build/aos-*`, `build/both-*`, `build-number/android/*` 원격 태그에서 최대 번호를 찾고 그 다음 번호를 `build-number/android/<number>` 태그로 먼저 원자적으로 예약합니다. 다른 브랜치나 동시 요청도 같은 번호를 사용할 수 없으며 예약 후 로컬 저장 커밋이나 push가 실패하면 안전을 위해 예약 번호를 재사용하지 않아 번호가 건너뛸 수 있습니다. Android working request는 APK와 Store 미업로드로 강제됩니다. 전체 요청이 성공하면 같은 Unity runner가 새 APK를 로컬 경로에서 Slack으로 직접 전송하므로 GitHub APK Artifact나 별도 delivery runner를 사용하지 않습니다. iOS working request는 마케팅 버전을 유지하고 빌드 번호를 `1`부터 시작합니다. CI가 같은 마케팅 버전의 완료된 빌드와 진행 중 업로드를 조회해 이미 사용된 최대 번호보다 1 큰 값을 작업용 요청에 적용한 뒤 TestFlight에 올립니다. 원본 BuildRequest와 BuildSettingsSO의 번들 번호는 변경하지 않습니다.

## BuildRequest schema 12 안내
...
5. `Version Info`에서 Version과 Build Number 표시를 확인합니다. Build Number는 `BuildSettingsSO.bundleNo`입니다.
...
8. `Development Build`를 확인합니다. 기본 OFF이며 켜면 `Build Number` 입력이 비활성화됩니다. Android/Both BuildCommit은 저장소 전체 Android 빌드·예약 태그의 최대 번호보다 1 큰 값을 원격에 먼저 예약한 뒤 BuildSettingsSO, BuildRequest, 커밋과 빌드 태그에 함께 저장합니다. 따라서 브랜치와 앱 버전이 달라도 Android 번호가 계속 증가하고 동시 요청도 중복되지 않습니다. iOS는 마케팅 버전마다 TestFlight 빌드번호를 1부터 자동 증가시킵니다. CI Android/iOS 빌드 옵션에는 `BuildOptions.Development`가 추가되고 Android APK는 Slack으로 직접 전달됩니다.
...
11. 모든 `unity-mobile` runner가 접근하는 공유 `CI_SECRET_ROOT/shared`에 `slack-bot-token`, `slack-channel-id`를 한 번 등록합니다. Bot에는 `chat:write`, `files:write` 권한과 대상 채널 참여가 필요합니다. `slack-webhook-url`은 구버전 호환용 파일이며 현재 알림에는 사용하지 않습니다. MacBook runner는 Mac Studio SMB 공유를 고정 경로에 먼저 마운트하고 runner 환경의 `CI_SECRET_ROOT`가 그 경로를 가리키게 합니다.
...
Development Android APK는 Android/iOS, 지연 Store 업로드, 최종 cleanup과 실패 집계가 모두 성공한 뒤 같은 `mobile-build` job에서 Slack external upload API로 직접 전송합니다. 시작·실패·iOS-only 성공 메시지는 동일한 Bot과 `slack-channel-id`의 `chat.postMessage`로 전송하고, APK 성공 메시지는 파일 게시물의 `initial_comment`에 포함해 중복 성공 메시지를 보내지 않습니다. APK 누락이나 확정된 Slack 업로드 실패는 Unity 원본 빌드 결과를 변경하지 않고 같은 채널에 `BUILD SUCCESS / APK DELIVERY FAILED` 메시지를 시도합니다. `chat.postMessage`가 `ok:false`를 반환하면 전체 응답 대신 정제된 Slack `error` 코드만 warning에 기록하므로 `not_in_channel`, `missing_scope`, `invalid_auth`, `channel_not_found` 등을 구분할 수 있습니다. Slack API 호출은 bounded timeout을 사용하며, 공유 `state/slack-apk-delivery`의 run별 영수증이 GitHub 재실행과 runner 변경에서도 중복 게시를 차단합니다. Slack 완료 결과가 불명확하면 pending 영수증을 보존하고 모순되는 실패 메시지를 보내지 않으며 운영자가 Slack 게시 여부를 확인한 뒤에만 해소합니다. Store 업로드에 성공한 AAB/IPA는 해당 upload step의 정확한 outcome으로 판정해 GitHub에 중복 보관하지 않습니다. 이번 phase marker 이후 staging된 Store 실패 복구 산출물은 3일, 실패 진단 로그는 7일만 보존합니다. 이 복구용 Artifact는 APK 전달과 무관하며 quota나 전송 오류를 성공으로 숨기지 않습니다.
...
- `com.actionfit.buildsetting@1.1.13` 이상
- `com.actionfit.githubauth@1.0.9` 이상
- `com.actionfit.customsymbols@1.0.9` 이상
- Unity `6000.2`
- Android/iOS 빌드용 Unity modules
- GitHub Actions self-hosted macOS runner 환경
