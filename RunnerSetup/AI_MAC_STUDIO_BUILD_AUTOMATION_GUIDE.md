...
All Android signing and upload credentials, team ids, Apple Distribution `.p12` files, App Store provisioning profiles, and optional keychain settings come exclusively from the local bundle selected by the `CI_SECRET_ROOT` environment variable. The scripts default to `$HOME/ci-secrets/build-automation`. BuildRequest never carries credential values or runner-local paths.
...
- If Android Unity signing fails, check required runner values `ANDROID_KEYSTORE_PATH`, `ANDROID_KEYSTORE_PASS`, and `ANDROID_KEYALIAS_PASS`; optionally check `ANDROID_KEYALIAS_NAME` or the request's non-secret `androidKeyaliasName`. Never diagnose signing by adding keystore bytes/passwords to request JSON.
...
- Do not paste Android keystore bytes/passwords, Google Play, App Store Connect, certificate, or keychain credential values into `.build/build_request.json`. Legacy schema 10 credential fields are ignored, not supported as fallback.
